...
=== Fields
These are the fields you are going to see in the body of a request in a GET or in the payload for POST or PUT
operations. This is an example of a Crispr plan.
...
|typeName
a|Name of the type of the plan. Possible values are:
...
a|Attempt type. Possible values are:
...
Curl:
include::{snippets}/outcomes/postCrisprPlan/http-request.adoc[]
